...
-(UITableViewCell*)setupReactModuleCell:(UITableView *)tableView data:(NSDictionary*)data indexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
      ...
      NSLog(@"newCellrenderinlist");
        ...
        //使用模板渲染 所以 new 之后  马上 update 触发渲染视图
...
{
  return objc_getAssociatedObject(self, _MJRefreshHeaderKey);
}
- (void)setMj_header:(MJRefreshHeader *)mj_header
{
  if (mj_header != self.mj_header) {
    ...
    objc_setAssociatedObject(self, _MJRefreshHeaderKey, mj_header, OBJC_ASSOCIATION_ASSIGN);
...
{
  return objc_getAssociatedObject(self, &MJRefreshFooterHeight);
}
...
- (void)setMj_footer:(MJRefreshFooter *)mj_footer
{
  if (mj_footer != self.mj_footer) {
    ...
    objc_setAssociatedObject(self, &MJRefreshFooterHeight,
